...
with tf.Session() as sess:
    for i in range(len(input_data)):
        print('from data array: y=' + str(sess.run(y)))

# 2. 读取文本文件里的数据。

# 创建文本文件作为本例的输入。
with open("../logs/imageDeal/test1.txt", "w") as file:
    file.write("File1, line1.\n")
    file.write("File1, line2.\n")
with open("../logs/imageDeal/test2.txt", "w") as file:
    file.write("File2, line1.\n")
    file.write("File2, line2.\n")

# 从文本文件创建数据集。这里可以提供多个文件。
input_files = ["../logs/imageDeal/test1.txt", "../logs/imageDeal/test2.txt"]
...
with tf.Session() as sess:
    for i in range(4):
        print('from file, line=' + str(sess.run(x)))
...
input_files = ["../logs/imageDeal/mnist_train.tfrecords"]
...
with tf.Session() as sess:
    ...
    sess.run(iterator.initializer,
             feed_dict={input_files: ["../logs/imageDeal/mnist_train.tfrecords"]})
    # 遍历所有数据一个epoch。当遍历结束时，程序会抛出OutOfRangeError。
    while True:
        try:
            x, y = sess.run([image, label])
            print(f'from tfrecords(placeholder): y={y}')
